...
--password 12345678 \
--delete-target-dir \
--target-dir /data/wangmeng/nshop/ods/ods_nshop_02_order_detail/bdp_day=${bdp_day} \
--num-mappers 1 \
--query 'SELECT * FROM order_detail WHERE FROM_UNIXTIME(order_detail_ctime/1000,"%Y%m%d")='${bdp_day}' and $CONDITIONS' 

/home/framework/hive-2.1.1/bin/hive -e "alter table 1902_wm_ods_nshop.ods_nshop_02_order_detail add partition(bdp_day='${bdp_day}')"
